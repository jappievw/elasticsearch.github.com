...
p. With the release of elasticsearch @0.90.4@ the completion suggester also supports fuzzy queries - this means, you can actually have a typo in your search and still get results back.
...
p. The fuzzy query can take specific fuzzy parameters. The following parameters are supported:

* @edit_distance@: Maximum edit distance, defaults to @1@
* @transpositions@: Sets if transpositions should be counted as one or two changes, defaults to @true@
...
p. Note: If you want to stick with the default values, but still use fuzzy, you can either use @fuzzy: {}@ or @fuzzy: true@.
