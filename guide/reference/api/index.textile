...
h2. Human readable output

p. Statistics are returned in a format suitable for humans (eg @"exists_time": "1h"@ or @"size": "1kb"@) and for computers (eg @"exists_time_in_millis": 3600000`@ or @"size_in_bytes": 1024@).  The human readable values can be turned off by adding @?human=false@ to the query string. This makes sense when the stats results are being consumed by a monitoring tool, rather than intended for humans.  In 0.90, @human@ defaults to @true@, but will be changed to default to @false@ from version 1.0 onwards.
